<?php
    class model_fasilitas{
        ...
        function getData(){
            ...
            while($rsData = mysql_fetch_array($query)){
               $data[$index]['id'] = $rsData['id_fasilitas'];
               $data[$index]['fasilitas'] = ucwords($rsData['fasilitas']);
        ...
        function getSearchData($val){
            ...
            $query = mysql_db_query($db,"SELECT * FROM $this->table_name WHERE $this->primary_key = '$val' OR fasilitas LIKE '%$val%'",$dbconn);
    ...
    $model_fasilitas = new model_fasilitas(); // buat obj baru
    $model_fasilitas->setTableIdentity("fasilitas","id_fasilitas");
    $model_fasilitas->setConnection("../../setting/conn.php");
?>